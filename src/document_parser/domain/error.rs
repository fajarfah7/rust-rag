...
use crate::document_parser::ports::embedder::EmbedError;
    ...
    fn from(err: EmbedError) -> Self {
        match err {
            EmbedError::EmptyInput =>
                PdfParserError::EmbedFailed("empty input".into()),

            EmbedError::RateLimited =>
                PdfParserError::EmbedFailed("rate limited".into()),

            EmbedError::Transport(msg) =>
                PdfParserError::EmbedFailed(format!("transport error: {msg}")),

            EmbedError::Provider { message, .. } =>
                PdfParserError::EmbedFailed(message),

            EmbedError::InvalidResponse(msg) =>
                PdfParserError::EmbedFailed(msg),

            EmbedError::Unknown(msg) =>
                PdfParserError::UnknownError(msg),
        }
    }
}